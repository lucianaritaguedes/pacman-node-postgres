...
var PACMAN_MOVE_FN = -1;
var GHOST_BLINKY_MOVE_FN = -1;
var GHOST_INKY_MOVE_FN  = -1;
var GHOST_PINKY_MOVE_FN = -1;
var GHOST_CLYDE_MOVE_FN = -1;
var erin = -1
...
function initGame(newgame) { 
	...
	if (erin  === -1) {
		erin = setInterval(() => {
			...
			if (typeof PACMAN_MOVE_FN === "function") {
				PACMAN_MOVE_FN();
			}
		
			if (GHOST_BLINKY_MOVE_FN !== -1) {
				GHOST_BLINKY_MOVE_FN('blinky');
			}
			if (GHOST_INKY_MOVE_FN !== -1) {
				GHOST_INKY_MOVE_FN('inky');
			}
			if (GHOST_PINKY_MOVE_FN !== -1) {
				GHOST_PINKY_MOVE_FN('pinky');
			}
			if (GHOST_CLYDE_MOVE_FN !== -1) {
				GHOST_CLYDE_MOVE_FN('clyde');
			}
			//updateVarPac();
		}, 16);
	}
